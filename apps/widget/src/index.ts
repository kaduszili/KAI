...
;(function () {
  ...
  if (!projectId) {
    console.warn('[Bentevi] Missing data-project attribute.')
    return
  }
  ...
  function buildUI() {
    ...
    const displaySubtitle = headerSubtitle.trim() || 'Bentevi'
    ...
    footer.innerHTML = 'Powered by <a href="https://bentevi.tech" target="_blank" rel="noopener noreferrer" id="kai-footer-link">Bentevi</a>'
    ...
    // ── Bubble wrapper (holds FAB + sparkle badge) ────────────────────────────
    const bubbleWrap = document.createElement('div')
    bubbleWrap.id = 'kai-bubble-wrap'
    bubbleWrap.appendChild(bubbleEl)

    animBadgeEl = document.createElement('div')
    animBadgeEl.id        = 'kai-anim-badge'
    animBadgeEl.innerHTML = sparkleAnimSVG
    animBadgeEl.classList.toggle('visible', showAnimation)
    bubbleWrap.appendChild(animBadgeEl)
  ...
  function togglePanel() {
    ...
    // Restart animation from scratch on each close (reflow trick)
    if (!isOpen && showAnimation) {
      animBadgeEl.classList.remove('visible')
      void animBadgeEl.offsetWidth  // force reflow
      animBadgeEl.classList.add('visible')
    } else {
      animBadgeEl.classList.remove('visible')
    }
  ...
  async function init() {
    try {
      ...
      if (res.ok) {
        ...
        showAnimation        = (b.showAnimation          as boolean) === true
